...
namespace Scripts.MapSetup.Services
{
	...
	public class Formation : ClientPage
	{
		...
		void Start(){
			...
			mapView.GetComponent<MapView> ()._mapModel = StaticAllData.currentMap;
			currentUnit = BridgeData.unitRepo._unitsById.ElementAt(0).Value; 

			//deletes units from map
			StaticAllData.currentMap._formationUnits.Clear();
		...
		void Update(){
			if (Input.GetMouseButtonDown (0) && pickedUpUnit == true) {
				...
				WorldPosition mouseWP = new WorldPosition (mouseVect.x * 3, mouseVect.z * 3); //*5 is callibrate
		...
		public void ChangeParty(){
			...
			case MapModel.Party.P1:
				currentParty = MapModel.Party.P2;
				break;
			case MapModel.Party.P2:
				currentParty = MapModel.Party.P3;
				break;
			case MapModel.Party.P3:
				currentParty = MapModel.Party.P4;
				break;
