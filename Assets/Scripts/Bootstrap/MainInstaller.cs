...
//this is loaded up in the scene. battle installer, menu installer, etc. these start up from scene context.

//why do the ifactories exist? to make it so that it can run from the processor,
//and not instantly. It creates a "factory" which can be added to the command processor,
//so that when that can is reached it is executed and the thing is created. 
//for the unit creation it is created instantly, and I think the fact it is 


//need to use factory classes to resolve all the dependencies: the factory creates the object with the dependencies.
//and the static classes need to become zenject singletons. 
...
namespace Bootstrap
{
    public class MainInstaller : MonoInstaller
    {
        ...
        private void InstallCommands()
        {
			...
			//Container.BindIFactory<StatChangeData, ReviveCommand> (); not used right now
			Container.BindIFactory<StatChangeData, DieCommand> ();
			Container.BindIFactory<IStatChange, StatChangeData, StatChangeCommand> ();

			Container.BindIFactory<StatChangeData, HPChangeCommand> ();
			Container.BindIFactory<StatChangeData, ResurrectCommand> ();


			Container.BindIFactory<IProjectile, RemoveProjectileCommand> ();

			Container.BindIFactory<UnitModel, MovementAbility> ();

			Container.Bind<ProjectileRepository.ProjectileFactory> ().AsSingle();
		...
		//this is what actually binds the other to the constructors in the classes.
		//it will appear as null unless the class is here otherwise Zneject cannot know which calss to
        private void InstallServices()
        {
            BindService<CommandProcessor>();
            BindService<UnitManager>();
            //BindService<LevelLoaderService>();
			BindService<LevelLoadMapService>();
			BindService<TickService> ();
			BindService<ProjectileManager> ();
			BindService<ProjectileRepository> ();
			BindService<HistoryManager> ();
			BindService<BattleEndCheckCommand>();
			//BindService<MapManagerNav> ();  monobehavior
