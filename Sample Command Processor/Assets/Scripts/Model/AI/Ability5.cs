...
namespace Model.AI
{
	public class Ability5 : GameCommand
	{
		...
		public Ability5(UnitModel unit, WorldModel world, TickService tickService,  
		...
		public override GameCommandStatus FixedStep()
		{
			...
			if (target != null){						//if target is not null
				if (WorldPosition.DistanceSq (_unit.Position, target.Position) < 20) { //if the current target meets criteria
			...
			// dvscode: _unit.targetPrioritiesHack [5] = target;   //THIS IS PART OF THE hack
