...
namespace Model.Abilities
{
	public class CommanderAbility2 : AbilityCommand  //must get the ability name exactly to the calss
	{
			...
			for (int i = 0; i < recruits.Length; i++) {
				if (recruits [i] == null) {
					...
					if (recruits[i] != null){ //should have a few more checks here including a range check
						var abilityToAdd = _abilityRepository.CreateAbilityFromId ("ability.formationTargetAbility2", _unit, 
	    ...
	    {
	        return string.Format("[CommanderAbility2,\n  recruits: {0},\n  formationTargets: {1}\n]", recruits.PrettyPrint(), formationTargets.PrettyPrint());
	    }
